...
async function seed() {
  await Promise.all(seedingData.forEach(async (data, idx) => {
    try {
      if (idx < 15) {
        console.log(idx +  "=========");
        await user_anggota.create({
          nama: data["Nama Pensiunan"] ? data["Nama Pensiunan"] : "-",
          password: data["NIP/NPP/NIK"] ? data["NIP/NPP/NIK"] : "-",
          no_induk: data["NIP/NPP/NIK"] ? data["NIP/NPP/NIK"] : "-",
