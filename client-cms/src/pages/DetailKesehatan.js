...
export default () => {
  ...
  const fetchApi = async () => {
    const { data } = await axios({
      method: "GET",
      url: `https://jatisejahtera-cms.herokuapp.com/data/claim-kesehatan/${dataId}`,
      //   url: `http://localhost:3001/data/claim-kesehatan/${dataId}`,
      headers: {
  ...
  const setuju = async () => {
    try {
      if (data) {
        await axios({
          method: "GET",
          url: `https://jatisejahtera-cms.herokuapp.com/data/pindah-status/kesehatan/${dataId}/${Number(data.status) + 1}`,
          //   url: `http://localhost:3001/data/pindah-status/kesehatan/${dataId}/${Number(data.status) + 1}`,
          headers: {
            token: localStorage.token,
          },
        });
        history.push("/claimkesehatan");
      }
    } catch (err) {
      let msg = "";
      if (err.response) {
      ...
      {loading ? (
        ...
        <Container>
          <div style={{ marginTop: 50 }}>
            ...
            <p style={{ textAlign: "center", fontWeight: "bold", fontSize: 30 }}>Pengajuan Claim Kesehatan</p>
          </div>
          <div style={{ marginBottom: 50 }}>
            ...
            <Form>
              <Form.Group as={Row}>
                ...
                <Col sm="10">
                  <Form.Control type="text" value={data.surat_permohonan_bantuan_biaya} disabled={true} />
                </Col>
              </Form.Group>
              <Form.Group as={Row}>
                <Form.Label column sm="2">
                ...
                <Col sm="10">
                  <Form.Control type="text" value={data.kuitansi_asli_rs} disabled={true} />
                </Col>
              </Form.Group>
              <Form.Group as={Row}>
                <Form.Label column sm="2">
                ...
                <Col sm="10">
                  <Form.Control type="text" value={data.surat_keterangan_rs} disabled={true} />
                </Col>
              </Form.Group>
              <Form.Group as={Row}>
                <Form.Label column sm="2">
